...
type Rule struct {
	...
	AlertName      string       `gorm:"uniqueIndex:idx_alert" json:"alertName"`                             // 告警名称
	CustomDesc     string       `gorm:"uniqueIndex:idx_alert" json:"desc"`                                  // 自定义描述
	MonitorMetrics string       `gorm:"uniqueIndex:idx_alert" json:"metrics"`                               // 监控指标
	AlarmThreshold string       `gorm:"uniqueIndex:idx_alert" json:"threshold"`                             // 告警阈值
	Forsearch      string       `gorm:"uniqueIndex:idx_alert" json:"duration"`                              // 触发时长
	Severity       string       `gorm:"uniqueIndex:idx_alert" json:"severity"`                              // 告警级别
	AlertLabel     string       `json:"alertLabel"`                                                         // 告警标签
	AlertTargets   []RuleTarget `gorm:"foreignKey:RuleId;constraint:OnDelete:CASCADE;" json:"alertTargets"` // 告警机器
	UserId         int          `json:"userId"`                                                             // 创建规则用户id
